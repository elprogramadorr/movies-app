...
export const ListItemComponent = ({ item, navigation }: { item: ListItem; navigation: any }) => {
  ...
  useEffect(() => {
    const fetchPosters = async () => {
      if (item.peliculas && item.peliculas.length > 0) {
        try {
          const lastMovieIds = item.peliculas.slice(-4).map((id) => parseInt(id, 10)); // Convierte los últimos 4 IDs a números
          ...
          console.error('Error al obtener los posters de las películas:', error);
        }
      }
    };
      ...
      return (
        <View style={styles.movieGrid}>
          {moviePosters.map((poster, index) => (
            <Image key={index} source={{ uri: poster }} style={styles.moviePoster} />
          ))}
        </View>
      );
    } else {
